...
class VolumeGroupRecoveryPoint(object):

    """A model that represents volume group recovery point properties.

    :param volume_group_recovery_point_ext_id: (:attr:`volume_group_recovery_point_ext_id`) The external identifier that can be used to retrieve the Volume Group recovery point using its URL.
    ...
    def __init__(self, volume_group_recovery_point_ext_id=None, consistency_group_ext_id=None, location_agnostic_id=None, volume_group_ext_id=None, volume_group_categories=None, *args, **kwargs):  # noqa: E501
