# Python Client For Nutanix Licensing Versioned APIs

The Python client for Nutanix Licensing Versioned APIs is designed for Python client application developers offering them simple and flexible access to APIs that licensing desc placeholder.
## Features
...
## Configuration
The python client for Nutanix Licensing Versioned APIs can be configured with the following parameters
...
```python
# Initialize the API
licenses_api_instance = LicensesApi(api_client=client) # client configured in previous step
extId = 'extId_example' # UUID.

# List license features
try:
    api_response = licenses_api_instance.list_features(
...
This library has a full set of [API Reference Documentation](https://developers.nutanix.com/sdk-reference?namespace=licensing&version=v4.0.a1&language=python). This documentation is auto-generated, and the location may change.
